...
const LEGACY_STORAGE_KEY = "perekodan-pbd-records";
let records = [];
...
async function loadRecordData() {
  try {
    ...
    setStatus(`Data TP Google Sheet gagal dimuat: ${reason}`, true);
  }
}
...
async function saveRecordBatchToStore(nextRecords) {
  if (!APPS_SCRIPT_URL) {
    setStatus("URL Apps Script belum ditetapkan.", true);
    return false;
  }
  ...
  try {
    ...
    // Gabungkan sementara dalam memori untuk paparan segera sahaja.
...
function clearLegacyLocalRecords() {
  try {
    ...
    console.warn("Salinan setempat lama tidak dapat dibuang:", error);
  }
}
